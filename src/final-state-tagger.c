...
/* the start state tagger) */
	...
	for (i=0;i<Darray_len(cRuleArray);i++) {
	  split_ptr = perl_split(Darray_get(cRuleArray,i));
...
/*Added by Golam Mortuza Hossain */
	    ...
	    my_ptr = Registry_get(lemma_hash, my_word) ;
            if ( my_ptr ) strcpy ( my_lemma, my_ptr);
	    else	
	    	{
	    	strcpy(my_word_lc, my_word); mylc(my_word_lc) ;
	    	my_ptr = Registry_get(lemma_hash, my_word_lc) ;
	...
	for (i=0;i<arraySize;i++)
	  {
	    ...
	    if (tag_corpus_array[i]!=staart && tag_corpus_array[i]!=new)
